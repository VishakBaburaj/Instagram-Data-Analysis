...
def load_data():
       ...
       return df_followers, df_posts, df_reels

# Create dataframes from the function
df_followers, df_posts, df_reels = load_data()

# Cleaning data to requirements
## Followers data
df_followers = df_followers[~(df_followers['Date_Time'] >= '2022-09-01')]
df_followers_asc = (df_followers.assign(Date=df_followers['Date_Time'].dt.date, Time=df_followers['Date_Time'].dt.time).sort_values(['Date','Time'], ascending=[True, True]))
...
## Posts data
df_posts_dsc = (df_posts.assign(Date=df_posts['Date_Time'].dt.date, Time=df_posts['Date_Time'].dt.time).sort_values(['Date','Time'], ascending =[False, False]))
df_posts_dsc['Date'] = pd.to_datetime(df_posts_dsc['Date'])
df_posts_dsc[['Profile Visits', 'Impressions', 'Follows', 'Accounts reached','Shares']] = df_posts_dsc[['Profile Visits', 'Impressions', 'Follows', 'Accounts reached','Shares']].fillna(value=0).astype(int)
...
df_reels_dsc = (df_reels.assign(Date=df_reels['Date_Time'].dt.date, Time=df_reels['Date_Time'].dt.time).sort_values(['Date','Time'], ascending =[False, False]))
df_reels_dsc['Date'] = pd.to_datetime(df_reels_dsc['Date'])
df_reels_dsc[['Instagram Shares', 'Instagram Saves', 'Facebook Play Count', 'Facebook Likes']] = df_reels_dsc[['Instagram Shares', 'Instagram Saves', 'Facebook Play Count', 'Facebook Likes']].fillna(value=0).astype(int)
df_reels_dsc[['Duration', 'Instagram and Facebook Plays', 'Instagram and Facebook Likes', 'Accounts reached', 'Instagram Plays', 'Instagram Likes', 'Instagram Comments']] = df_reels_dsc[['Duration', 'Instagram and Facebook Plays', 'Instagram and Facebook Likes', 'Accounts reached', 'Instagram Plays', 'Instagram Likes', 'Instagram Comments']].astype(int)
